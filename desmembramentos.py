...
def instalar_ferramentas() -> None:
    log.info("=" * 70)
    log.info("DEEP ORQUESTRADOR V13.0 — HASH + DINOv2 + ORB")
    log.info("=" * 70)

    # OBS.: em produção, prefira instalar estas dependências por requirements.txt.
    pacotes = [
        ("pandas", "pandas"),
        ("openpyxl", "openpyxl"),
        ("requests", "requests"),
...
# Métricas novas são APENAS comparativas nesta versão.
# As regras de status continuam usando Percentual_Similaridade_Atual.
DINO_MODEL_NAME: str = "facebook/dinov2-small"
DINO_BATCH_SIZE: int = 16
PESO_DINO_EXPLORATORIO: float = 0.65
PESO_ORB_EXPLORATORIO: float = 0.35
ORB_NFEATURES: int = 1600
...
def aplicar_status(
    ...
    prioridade_nova = PRIORIDADE_STATUS.get(novo_status, 0)
    prioridade_atual = PRIORIDADE_STATUS.get(resultado[idx]["status"], 0)
    ...
    if prioridade_nova >= prioridade_atual:
        ...
        # Guarda as métricas do mesmo par que gerou o apontamento exibido.
        if metricas:
            resultado[idx]["similaridade_antiga"] = metricas.get("antiga", -1.0)
            resultado[idx]["similaridade_dino"] = metricas.get("dino", -1.0)
            resultado[idx]["similaridade_orb"] = metricas.get("orb", -1.0)
            resultado[idx]["similaridade_nova"] = metricas.get("nova", -1.0)
            resultado[idx]["divergencia_metodos"] = metricas.get("divergencia", -1.0)
            resultado[idx]["foto_a_url"] = metricas.get("foto_a_url", "")
            resultado[idx]["foto_b_url"] = metricas.get("foto_b_url", "")
            resultado[idx]["foto_a_tipo"] = metricas.get("foto_a_tipo", "")
            resultado[idx]["foto_b_tipo"] = metricas.get("foto_b_tipo", "")
            resultado[idx]["par_fotos"] = metricas.get("par_fotos", "")
            resultado[idx]["matricula_a"] = metricas.get("matricula_a", "")
            resultado[idx]["matricula_b"] = metricas.get("matricula_b", "")
...
# =============================================================================
# 4. DOWNLOAD E EXTRAÇÃO DE RECURSOS VISUAIS
# =============================================================================
def baixar_recursos_imagem(url: str):
    """
    Baixa a foto uma única vez e extrai:
      - hashes antigos (pHash/aHash/dHash)
      - descritores/pontos ORB
      - uma cópia reduzida em memória para gerar o embedding DINOv2 depois
    """
    if not url:
        return url, None
    ...
    try:
        ...
        try:
            ...
            # ORB trabalha em escala de cinza. Limitamos o tamanho para acelerar.
            img_orb = img.copy()
            img_orb.thumbnail((1280, 1280))
            gray = cv2.cvtColor(np.asarray(img_orb), cv2.COLOR_RGB2GRAY)
            ...
            # Para o DINO não precisamos guardar a foto original gigante.
            img_dino = img.copy()
            img_dino.thumbnail((768, 768))
            buffer = BytesIO()
            img_dino.save(buffer, format="JPEG", quality=88, optimize=True)

            return url, {
                "hashes": hashes,
                "orb_pontos": pontos,
                "orb_descritores": descriptors,
                "dino_img_bytes": buffer.getvalue(),
                "dino": None,
            }
        except Exception:
            return url, "ERRO_LINK"
...
def pre_carregar_recursos(urls: set, cache: dict) -> None:
    urls_novas = {u for u in urls if u and str(u).strip() and u not in cache}
    if not urls_novas:
        return

    log.info("Baixando e extraindo HASH/ORB de %d fotos exclusivas...", len(urls_novas))
    max_workers = min(48, max(8, (os.cpu_count() or 4) * 4))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
        for url, recursos in ex.map(baixar_recursos_imagem, urls_novas):
            cache[url] = recursos


def pre_calcular_embeddings_dino(cache: dict, progresso=None) -> None:
    """Calcula um embedding DINOv2 por foto válida, em lotes."""
    import numpy as np
    import torch
    from PIL import Image
    from io import BytesIO
    ...
    itens = [
        ...
        if isinstance(rec, dict) and rec.get("dino_img_bytes") and rec.get("dino") is None
    ]
    if not itens:
        return

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log.info("Carregando DINOv2 (%s) em %s...", DINO_MODEL_NAME, device)
    ...
    with torch.inference_mode():
        for inicio in range(0, total, DINO_BATCH_SIZE):
            lote = itens[inicio:inicio + DINO_BATCH_SIZE]
            imagens = []
            urls_lote = []

            for url, rec in lote:
                try:
                    imagens.append(Image.open(BytesIO(rec["dino_img_bytes"])).convert("RGB"))
                    urls_lote.append(url)
                except Exception:
                    rec["dino"] = None

            if imagens:
                inputs = processor(images=imagens, return_tensors="pt")
                inputs = {k: v.to(device) for k, v in inputs.items()}
                outputs = model(**inputs)

                # CLS token = representação global da imagem.
                embeddings = outputs.last_hidden_state[:, 0, :]
                embeddings = torch.nn.functional.normalize(embeddings, p=2, dim=1)
                embeddings = embeddings.detach().cpu().numpy().astype(np.float32)

                for url, emb in zip(urls_lote, embeddings):
                    cache[url]["dino"] = emb

            # Libera a miniatura JPEG: a partir daqui só precisamos do embedding.
            for _, rec in lote:
                rec.pop("dino_img_bytes", None)

            if progresso:
                progresso(min(inicio + len(lote), total), total)

    # Evita manter modelo na memória do app após a etapa de extração.
    del model
    if torch.cuda.is_available():
        torch.cuda.empty_cache()


def _recurso_valido(rec) -> bool:
    return isinstance(rec, dict)


def _hash_do_recurso(rec):
    return rec.get("hashes") if _recurso_valido(rec) else rec


def calcular_similaridade_dino(rec_a, rec_b) -> float:
    import numpy as np
    if not (_recurso_valido(rec_a) and _recurso_valido(rec_b)):
        return -1.0
    a, b = rec_a.get("dino"), rec_b.get("dino")
    if a is None or b is None:
        return -1.0
    # Os vetores já estão normalizados; produto escalar = cosseno.
    sim = float(np.dot(a, b))
    return round(max(0.0, min(100.0, sim * 100.0)), 2)


def calcular_similaridade_orb(rec_a, rec_b) -> float:
    """
    Mede correspondência estrutural local.
    ORB encontra pontos; RANSAC exige coerência geométrica entre eles.
    """
    import cv2
    import numpy as np
    ...
    desc_a = rec_a.get("orb_descritores")
    desc_b = rec_b.get("orb_descritores")
    pts_a = rec_a.get("orb_pontos")
    pts_b = rec_b.get("orb_pontos")
    ...
    try:
        ...
        # Poucos matches não devem gerar 80/90% só por coincidência.
        if len(bons) < ORB_MIN_GOOD_MATCHES:
            return round(min(20.0, (len(bons) / ORB_MIN_GOOD_MATCHES) * 20.0), 2)

        origem = np.float32([pts_a[m.queryIdx] for m in bons]).reshape(-1, 1, 2)
        destino = np.float32([pts_b[m.trainIdx] for m in bons]).reshape(-1, 1, 2)
        _, mascara = cv2.findHomography(origem, destino, cv2.RANSAC, 5.0)

        if mascara is None:
            return round(min(35.0, len(bons) / 40.0 * 35.0), 2)

        inliers = int(mascara.ravel().sum())
        taxa_inliers = inliers / max(1, len(bons))
        suporte = min(1.0, inliers / 30.0)

        # A coerência geométrica pesa mais; quantidade de apoio evita falso 100%.
        score = 100.0 * (0.60 * taxa_inliers + 0.40 * suporte)
        return round(max(0.0, min(100.0, score)), 2)
    except Exception:
        return 0.0


def calcular_metricas_recursos(rec_a, rec_b) -> dict:
    antiga = calcular_similaridade(_hash_do_recurso(rec_a), _hash_do_recurso(rec_b))
    dino = calcular_similaridade_dino(rec_a, rec_b)
    orb = calcular_similaridade_orb(rec_a, rec_b)
    return {"antiga": antiga, "dino": dino, "orb": orb}


def consolidar_metricas(combinacoes: list[dict]) -> dict:
    """
    Consolida a comparação entre matrícula A x matrícula B.

    - O método antigo continua usando o MAIOR HASH entre todas as combinações.
    - Para auditoria visual, escolhemos UM par real de fotos (A? x B?) pelo
      maior score novo disponível. Assim a tela sempre consegue mostrar as
      imagens exatas responsáveis pela comparação exibida.
    """
    validas = [m for m in combinacoes if any(m.get(k, -1.0) >= 0 for k in ("antiga", "dino", "orb"))]
    if not validas:
        return {
            "antiga": -1.0, "dino": -1.0, "orb": -1.0, "nova": -1.0,
            "divergencia": -1.0, "foto_a_url": "", "foto_b_url": "",
            "foto_a_tipo": "", "foto_b_tipo": "", "par_fotos": "",
        }

    # Legado: mantém exatamente a ideia do maior HASH para não alterar regra.
    antigas = [m.get("antiga", -1.0) for m in validas if m.get("antiga", -1.0) >= 0]
    antiga_global = max(antigas) if antigas else -1.0

    # Calcula o score novo PARA CADA PAR DE FOTOS, sem misturar o DINO de um
    # par com o ORB de outro.
    enriquecidas = []
    for m in validas:
        dino = m.get("dino", -1.0)
        orb = m.get("orb", -1.0)
        if dino >= 0 and orb >= 0:
            nova = dino * PESO_DINO_EXPLORATORIO + orb * PESO_ORB_EXPLORATORIO
        elif dino >= 0:
            nova = dino
        elif orb >= 0:
            nova = orb
        ...
        x = dict(m)
        x["nova"] = nova
        enriquecidas.append(x)

    # Melhor par para mostrar: prioriza score novo; se indisponível, HASH.
    melhor = max(enriquecidas, key=lambda m: (m.get("nova", -1.0), m.get("antiga", -1.0)))
    dino = melhor.get("dino", -1.0)
    orb = melhor.get("orb", -1.0)
    nova = melhor.get("nova", -1.0)
    divergencia = abs(antiga_global - nova) if antiga_global >= 0 and nova >= 0 else -1.0

    return {
        "antiga": round(antiga_global, 2) if antiga_global >= 0 else -1.0,
        "dino": round(dino, 2) if dino >= 0 else -1.0,
        "orb": round(orb, 2) if orb >= 0 else -1.0,
        "nova": round(nova, 2) if nova >= 0 else -1.0,
        "divergencia": round(divergencia, 2) if divergencia >= 0 else -1.0,
        "foto_a_url": melhor.get("foto_a_url", ""),
        "foto_b_url": melhor.get("foto_b_url", ""),
        "foto_a_tipo": melhor.get("foto_a_tipo", ""),
        "foto_b_tipo": melhor.get("foto_b_tipo", ""),
        "par_fotos": melhor.get("par_fotos", ""),
    }
...
def _processar_par(args: tuple) -> list[tuple]:
    ...
    rp_A, ri_A = row_A["rec_prop"], row_A["rec_imov"]
    rp_B, ri_B = row_B["rec_prop"], row_B["rec_imov"]

    def _comparar(rec_a, rec_b, tipo_a, tipo_b, url_a, url_b):
        m = calcular_metricas_recursos(rec_a, rec_b)
        m.update({
            "foto_a_url": url_a or "",
            "foto_b_url": url_b or "",
            "foto_a_tipo": tipo_a,
            "foto_b_tipo": tipo_b,
            "par_fotos": f"{tipo_a} x {tipo_b}",
        })
        return m

    combinacoes = [
        _comparar(rp_A, rp_B, "A1", "B1", row_A.get("url_prop", ""), row_B.get("url_prop", "")),
        _comparar(rp_A, ri_B, "A1", "B2", row_A.get("url_prop", ""), row_B.get("url_imov", "")),
        _comparar(ri_A, rp_B, "A2", "B1", row_A.get("url_imov", ""), row_B.get("url_prop", "")),
        _comparar(ri_A, ri_B, "A2", "B2", row_A.get("url_imov", ""), row_B.get("url_imov", "")),
    ]
    metricas = consolidar_metricas(combinacoes)
    metricas["matricula_a"] = row_A["id_completo"]
    metricas["matricula_b"] = row_B["id_completo"]

    # IMPORTANTE: decisões atuais continuam 100% baseadas no método antigo.
    sim_max = metricas["antiga"] if metricas["antiga"] >= 0 else 0.0
    sims_validas = [m["antiga"] for m in combinacoes if m["antiga"] >= 0.0]
    ...
    if url_back_a and url_back_b and url_back_a == url_back_b and sim_max < 40.0:
        if dist is not None and dist <= LIMITE_MESMO_LOCAL:
            ...
            updates.append((ia, "🚨 FRAUDE DE BACKOFFICE - FOTOS DISTINTAS NO MESMO LOCAL", msg, row_B["id_completo"], sim_max, "", metricas))
            updates.append((ib, "🚨 FRAUDE DE BACKOFFICE - FOTOS DISTINTAS NO MESMO LOCAL", msg, row_A["id_completo"], sim_max, "", metricas))
            ...
            updates.append((ia, "🚨 FRAUDE GRAVE - OUTRO IMÓVEL NO MESMO CÓDIGO", msg, row_B["id_completo"], sim_max, "", metricas))
            updates.append((ib, "🚨 FRAUDE GRAVE - OUTRO IMÓVEL NO MESMO CÓDIGO", msg, row_A["id_completo"], sim_max, "", metricas))

    for idx_p, row_p, row_o in [(ia, row_A, row_B), (ib, row_B, row_A)]:
        cod_o = str(row_o["id_completo"])
        if mesmo_codigo:
            imagem_diferente = bool(sims_validas) and (sim_max < SIM_DESMEMBRAMENTO)
            ...
            if imagem_diferente or coord_diferente:
                ...
                updates.append((idx_p, "🚨 DESMEMBRAMENTO - FOTO INTRUSA",
                                detalhe, cod_o, sim_max, cod_o, metricas))
        else:
            coord_proxima = (dist is not None) and (dist <= DIST_DUPLICATA)
            imagem_parecida = bool(sims_validas) and (sim_max >= SIM_DUPLICATA)
            ...
            if coord_proxima and imagem_parecida:
                updates.append((idx_p, "🚨 DUPLICATA - MESMO IMÓVEL EM OUTRO CÓDIGO",
                                ...
                                f"({sim_max:.1f}% sim, {int(dist)}m).", cod_o, sim_max, "", metricas))
            elif inconsistencia_parcial:
                updates.append((idx_p, "👀 REVISÃO - FOTO PARECIDA",
                                f"Possível ângulo diferente: {cod_o} ({sim_max:.1f}% sim).",
                                cod_o, sim_max, "", metricas))
            elif dist is not None and dist <= 10:
                updates.append((idx_p, "⚠️ ALERTA DE PROXIMIDADE",
                                f"GPS muito colado ({int(dist)}m) a outro cadastro.",
                                cod_o, sim_max, "", metricas))
...
def processar_desmembramentos(arquivo_entrada, progresso=None):
    ...
    pre_carregar_recursos(urls_dl, cache_recursos)
    atualizar(0.54, "Extraindo estrutura", "HASH e ORB calculados; iniciando DINOv2")

    def _progresso_dino(feitos, total):
        if total:
            p = 0.54 + 0.16 * (feitos / total)
            atualizar(p, "DINOv2", f"{feitos:,}/{total:,} imagens processadas".replace(",", "."))

    pre_calcular_embeddings_dino(cache_recursos, progresso=_progresso_dino)
    ...
    for i in df.index:
        ...
        dados_rows[i] = {
            ...
            "lat": row["Lat_Num"],
            "lon": row["Lon_Num"],
            "rec_prop": cache_recursos.get(row["URL_Prop"]),
            "rec_imov": cache_recursos.get(row["URL_Imov"]),
            "url_prop": str(row["URL_Prop"]).strip() if pd.notna(row["URL_Prop"]) else "",
            "url_imov": str(row["URL_Imov"]).strip() if pd.notna(row["URL_Imov"]) else "",
    ...
    resultado: dict[int, dict] = {
        i: {
            ...
            "similaridade": -1.0,
            "desmembramento": "",
            "similaridade_antiga": -1.0,
            "similaridade_dino": -1.0,
            "similaridade_orb": -1.0,
            "similaridade_nova": -1.0,
            "divergencia_metodos": -1.0,
            "foto_a_url": "",
            "foto_b_url": "",
            "foto_a_tipo": "",
            "foto_b_tipo": "",
    ...
    log.info("Analisando pares visualmente (HASH atual + DINOv2 + ORB)...")
    if pares:
        # ThreadPool mantém o cache de embeddings/descriptors compartilhado,
        # evitando copiar matrizes grandes para processos separados.
        max_workers = min(32, max(4, (os.cpu_count() or 4) * 2))
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            for updates in executor.map(_processar_par, args_pares, chunksize=256):
                for idx_p, novo_status, detalhe, cod, sim, desmemb, metricas in updates:
                    aplicar_status(
                        resultado, idx_p, novo_status, detalhe, cod,
                        similaridade=sim,
    ...
    for i, row in df.iterrows():
        if resultado[i]["status"] != "✅ OK":
            continue
        rp = cache_recursos.get(row["URL_Prop"])
        ri = cache_recursos.get(row["URL_Imov"])
        sem_foto_valida = (rp is None or isinstance(rp, str)) and (ri is None or isinstance(ri, str))
        sem_gps = pd.isna(row["Lat_Num"])
        if sem_foto_valida:
            if (row["URL_Prop"] or row["URL_Imov"]) and (isinstance(rp, str) or isinstance(ri, str)):
    ...
    # Métricas paralelas para auditoria.
    def _fmt_metrica(i, chave):
        valor = resultado[i].get(chave, -1.0)
        return f"{valor:.2f}%" if valor is not None and valor >= 0 else ""

    df["Similaridade_Atual_HASH"] = [_fmt_metrica(i, "similaridade_antiga") for i in df.index]
    df["Similaridade_DINOv2"] = [_fmt_metrica(i, "similaridade_dino") for i in df.index]
    df["Similaridade_ORB_Estrutural"] = [_fmt_metrica(i, "similaridade_orb") for i in df.index]
    df["Similaridade_Nova_Experimental"] = [_fmt_metrica(i, "similaridade_nova") for i in df.index]
    df["Divergencia_Atual_vs_Nova"] = [_fmt_metrica(i, "divergencia_metodos") for i in df.index]
    df["Auditoria_Foto_A"] = [resultado[i].get("foto_a_url", "") for i in df.index]
    df["Auditoria_Foto_B"] = [resultado[i].get("foto_b_url", "") for i in df.index]
    df["Auditoria_Tipo_Foto_A"] = [resultado[i].get("foto_a_tipo", "") for i in df.index]
    df["Auditoria_Tipo_Foto_B"] = [resultado[i].get("foto_b_tipo", "") for i in df.index]
    df["Auditoria_Melhor_Par"] = [resultado[i].get("par_fotos", "") for i in df.index]
    df["Auditoria_Matricula_A"] = [resultado[i].get("matricula_a", "") for i in df.index]
    df["Auditoria_Matricula_B"] = [resultado[i].get("matricula_b", "") for i in df.index]
